...
function check_wsample_wrep(a::AbstractArray, vrgn, wv::AbstractVector, ptol::Real;
...
using Statistics: direct_sample!, alias_sample!
...
function check_wsample_norep(a::AbstractArray, vrgn, wv::AbstractVector, ptol::Real;
                             ordered::Bool=false, rev::Bool=false)
...
import Statistics: naive_wsample_norep!, efraimidis_a_wsample_norep!,
...
a = sample(4:7, 3; weights=wv, replace=false, ordered=false)
check_wsample_norep(a, (4, 7), wv, -1; ordered=false)

for rev in (true, false), T in (Int, Int16, Float64, Float16, BigInt, ComplexF64, Rational{Int})
    r = rev ? reverse(4:7) : (4:7)
    r = T===Int ? r : T.(r)
    aa = Int.(sample(r, 3; weights=wv, replace=false, ordered=true))
