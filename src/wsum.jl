...
if VERSION >= v"1.4.0"
    ...
    _sum(a::AbstractArray; dims=:, weights::Union{AbstractArray,Nothing}=nothing) =
        _sum(a, dims, weights)
    _sum(f, a::AbstractArray; dims=:, weights::Union{AbstractArray,Nothing}=nothing) =
        _sum(f, a, dims, weights)
    _sum(a::AbstractArray, ::Colon, weights) = _sum(identity, a, :, weights)
    _sum(a::AbstractArray, ::Colon, ::Nothing) = sum(identity, a)
    _sum(f, a, ::Colon, ::Nothing) = mapreduce(f, add_sum, a)

    _sum!(r::AbstractArray, A::AbstractArray;
          init::Bool=true, weights::Union{AbstractArray,Nothing}=nothing) =
        _sum!(identity, r, A; init=init, weights=weights)
    _sum!(f::Function, r::AbstractArray, A::AbstractArray;
          init::Bool=true, weights::Union{AbstractArray,Nothing}=nothing) =
    ...
    _sum(A::AbstractArray{T}, dims::Colon, w::AbstractArray{T}) where {T<:BlasReal} =
        dot(vec(A), vec(w))
